...
class SettingFragment : Fragment() {
    ...
    private lateinit var alarmToggle: Switch //알람설정 스위치
    ...
    override fun onCreateView(
        ...
        //알람설정 스위치 상태에 따라 텍스트 설정
        alarmToggle = view.findViewById(R.id.alarmToggle)
        alarmToggle.text = if (alarmToggle.isChecked) "on" else "off"
        alarmToggle.setOnCheckedChangeListener { _, isChecked ->
            alarmToggle.text = if (isChecked) "on" else "off"
        }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        if (cursor.moveToNext()) {
            ...
            val friendIndex = cursor.getColumnIndex("friendCount")
            val friendCount = cursor.getInt(friendIndex)
            ...
            val tv_lvNum = view.findViewById<TextView>(R.id.tv_lvNum)
            tv_lvNum.text = ikkiLevel.toString()
            val tv_todoNum = view.findViewById<TextView>(R.id.tv_todoNum)
            tv_todoNum.text = todoCount.toString()
            val tv_friendNum = view.findViewById<TextView>(R.id.tv_friendNum)
            tv_friendNum.text = friendCount.toString()
        ...
        profileEditButton.setOnClickListener {
            ...
            dialogLayout.findViewById<Button>(R.id.btn_confirm).setOnClickListener {
                if (checkPassword(password)) {
                    ...
                    val profileEdit = Intent(requireActivity(), SettingEditActivity::class.java)
